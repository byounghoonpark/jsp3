 ...
 --%>
	...
	<form method="post" name="f">
		...
		담당업무: <input type="checkbox" name="job" value="서무"
			<c:if test="${fn:contains(jobs,'서무')}">checked</c:if>>서무 <input
			type="checkbox" name="job" value="개발"
			<c:if test="${fn:contains(jobs,'개발')}">checked</c:if>>개발 <input
			type="checkbox" name="job" value="비서"
			<c:if test="${fn:contains(jobs,'비서')}">checked</c:if>>비서 <input
			type="checkbox" name="job" value="유지보수"
			<c:if test="${fn:contains(jobs,'유지보수')}">checked</c:if>>유지보수<br>
		<input type="submit" value="전송">
		<h3>파라미터 값 출력하기</h3>
		이름 : ${param.name }<br> 입사일 : ${param.hiredate }<br> 급여 :
		${param.salary }<br> 담당업무 : ${param.job }<br>
